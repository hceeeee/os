...
// 通用的数字转输出
static void print_number(long num, int base, int sign, putc_fn out, void *ctx) {
    char buf[32];
    int i = 0;
    unsigned long n;
...
static void vformat(const char *fmt, va_list ap, putc_fn out, void *ctx) {
    for (const char *p = fmt; *p; p++) {
        ...
        p++;
        switch (*p) {
            case 'd': print_number(va_arg(ap,int),10,1,out,ctx); break;
            case 'x': print_number(va_arg(ap,int),16,0,out,ctx); break;
            case 's': {
...
static void console_out(char c, void *ctx) { console_putc(c); }
